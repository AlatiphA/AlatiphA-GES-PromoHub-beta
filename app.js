...
/* =========================
   ...
   ─────────────────────────
   Keys follow format: PROMOHUB-XXXX-YYYY
   ...
   NOTE: Uses a DIFFERENT multiplier (13) and
   modulus (8951) than GES Pasco (7, 9973) so
   keys are not interchangeable between apps.
========================= */

function supporterChecksum(code) {
  let sum = 0;
  for (let i = 0; i < code.length; i++) {
    sum += code.charCodeAt(i) * (i + 13);
  }
  return (sum % 8951).toString(36).toUpperCase().padStart(4, "0");
}

function isValidSupporterKey(key) {
  const clean = key.trim().toUpperCase();
  const match = clean.match(/^PROMOHUB-([A-Z0-9]{4,8})-([A-Z0-9]{4})$/);
...
function isSupporter() {
  return localStorage.getItem("promohub_isSupporter") === "true";
}
...
if (supporterKeySubmit) {
  supporterKeySubmit.addEventListener("click", () => {
    ...
    if (isValidSupporterKey(key)) {
      localStorage.setItem("promohub_isSupporter", "true");
      supporterKeyModal.classList.remove("open");
      applySupporterUI();
    } else {
